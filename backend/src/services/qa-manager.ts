...
export class QAManager {
    ...
    async create(data: Partial<QAPair>): Promise<DatabaseResponse<QAPair>> {
        return this.dataProvider.createDocument<QAPair>(this.collectionName, data as Omit<QAPair, '_id'>);
    }
    ...
    async getQaPair(id: string): Promise<DatabaseResponse<QAPair>> {
        const response = await this.dataProvider.getDocuments<QAPair>(this.collectionName, { _id: id });
     ...
     * Update a QA pair
     */
    async updateQaPair(id: string, question: string, answer: string): Promise<DatabaseResponse<QAPair>> {
        return this.dataProvider.updateDocument<QAPair>(this.collectionName, id, { question, answer });
    }
